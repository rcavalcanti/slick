...
class HoistClientOps extends Phase {
  ...
  def apply(state: CompilerState) = state.map { tree =>
    ClientSideOp.mapResultSetMapping(tree, false) { case rsm @ ResultSetMapping(_, ss, _) =>
      ...
      val defs1m = defs1.zipWithIndex.map { case ((s, _), i) => (s, ElementSymbol(i+1)) }.toMap
      ...
      val rsmProj2 = rsmProj.replace {
        case Select(r @ Ref(s), f) if s == base =>
          Select(r, defs1m(f))
      }
      val rsm2 = ResultSetMapping(base, rewriteDBSide(rsmFrom), rsmProj2).nodeWithComputedType(SymbolScope.empty, false, true)
