...
class NewQuerySemanticsTest extends AsyncTest[RelationalTestDB] {
  ...
  def testNewFusion = {
    ...
    DBIO.seq(
      as.schema.create,
      as ++= data,
      as.result.map(_.toSet shouldBe data),
      q1.result.map(_.toSet shouldBe data.zip(data)),
      q2.result.map(_.toSet shouldBe data.zip(data).zip(data)),
      q3.result.map(_.toSet shouldBe data),
      q4.result.map(_.toSet shouldBe data.filter(_._3 == "b").map { case t @ (id, _, _) => (id, t) }),
      q5a.result.map(_ shouldBe Set(2, 3)),
      q5b.result.map(_ shouldBe Set(2, 3)),
      q5c.result.map(_ shouldBe Set(2, 3)),
      q6.result.map(_.toSet shouldBe Set((("c","b"),3,3,3,Some(3),3), (("a","a"),3,3,3,Some(1),3), (("a","b"),3,3,3,Some(2),3))),
      q7.result.map(_.toSet shouldBe Set(1, 2)),
      q8.result.map(_ shouldBe Seq("c", "a", "a")),
      q9a.result.map(_ shouldBe Seq(3, 1, 2)),
      q9b.result.map(_ shouldBe Seq(3, 1, 2)),
      q10.result.map(_ shouldBe Seq(1, 2, 3, 4, 5, 6, 7, 8, 9)),
      q11a.result.map(_ shouldBe Seq(1, 2, 3, 4, 5)),
      q11b.result.map(_ shouldBe Seq(1, 2, 3)),
      q11c.result.map(_ shouldBe Seq(2, 3)),
      q11d.result.map(_ shouldBe Seq(2, 3, 4)),
      q11e.result.map(_ shouldBe Seq(8, 9)),
      q11f.result.map(_ shouldBe Seq(3, 4, 6)),
      q12.result.map(_ shouldBe Seq("a", "a"))
    )
  }
